...
// global constexpr int n = 8;
// global constexpr int n = 7;
global int arr[n][n] = zero;

func int do_thing(s_v2i index, int target_val, s_v2i end_pos)
{
	int result = 0;
	...
	for(int i = 0; i < 4; i += 1) {
		...
		if(is_valid_index(pos, n, n)) {
			if(arr[pos.y][pos.x] == target_val) {
				int temp = do_thing(pos, target_val + 1, end_pos);
				result += temp;
			}
		}
	}
...
int main()
{
	...
	for(int line_i = 0; line_i < line_count; line_i += 1) {
		...
		for(int i = 0; i < n; i += 1) {
			...
			if(arr[line_i][i] == 0) {
				zero_arr.add({v2i(i, line_i)});
			}
			if(arr[line_i][i] == 9) {
				nine_arr.add({v2i(i, line_i)});
			}
		}
	}
	...
	foreach_val(zero_i, zzero, zero_arr) {
		foreach_val(nine_i, nine, nine_arr) {
			result += do_thing(zzero.pos, 1, nine.pos);
		}
	}
